...
export function registerQuizHandler(
  ...
  const fetchQuiz = async (data) => {
    ...
    if (CONFIG.DEBUG) {
      ...
      console.log(
        `[START_QUIZ] Quiz: ${JSON.stringify(
          sessions.get(sessionCode).quizzes[0]
        )}`
      );
    }
  };
  ...
  const validateAnswer = async (data) => {
    ...
    if (CONFIG.DEBUG) {
      console.log(
        `[SUBMIT_ANSWER] User ${userId} from session ${sessionCode} submitted answer: ${data.opt} -> ${isCorrect}`
      );
      console.log(
        `[SUBMIT_ANSWER] Quiz: ${JSON.stringify(
          sessions.get(sessionCode).quizzes[0]
        )}`
      );
    }
  ...
  const sendNextQuiz = async (data) => {
    ...
    if (CONFIG.DEBUG) {
      console.log(
        `[NEXT_QUIZ] Time up. Sending next quiz to session ${sessionCode}`
      );
  ...
  const sendResult = async (data) => {
    ...
    if (userId === sessions.get(sessionCode).host) {
      ...
      if (CONFIG.DEBUG) {
        console.log(
          `[QUIZ_RESULT] Quiz result sent to attendee ${userId} for session ${sessionCode}`
        );
